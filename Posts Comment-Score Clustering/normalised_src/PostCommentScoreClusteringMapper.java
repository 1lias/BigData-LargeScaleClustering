...
public class PostCommentScoreClusteringMapper extends Mapper<Object, Text, DoubleDoublePair, DoubleDoublePair> {
  ...
  @Override
	public void map(Object key, Text data, Context context)
       ...
       commentCount_Score = new DoubleDoublePair(Double.parseDouble(userMap.get("Age")),
                                                 Double.parseDouble(userMap.get("Reputation")));

       double centroidX = centroids.get(0).getX().get();
       double centroidY = centroids.get(0).getY().get();

       double dataPointX = ((commentCount_Score.getX().get()) - 13)/81;
       double dataPointY = ((commentCount_Score.getY().get()) - 1)/640236;
